...
class CmdChannelGenerator:
    ...
    FILTER_CONFIG = {
        'FZ': {'cutoff_hz': 1, 'fs': 100, 'order': 2}
    }
    
    @classmethod
    def create_cmd_channels(cls, channels: List[str], units: List[str], 
                           data: np.ndarray, unit_system: str, 
                           sign_convention: str) -> Tuple[List[str], List[str], np.ndarray]:
        """
        ...
        try:
            ...
            result_data = ConventionConverter.convert_channel_convention(
                channels, data_sae, current_convention='SAE',
    ...
    @classmethod
    def _validate_inputs(cls, channels: List[str], units: List[str],
                        data: np.ndarray, unit_system: str) -> bool:
    ...
    @classmethod
    def _generate_cmd_channels(cls, channels: List[str], units: List[str],
                               data: np.ndarray, unit_system: str,
                               existing: List[str]) -> Tuple[List[str], List[str], List[np.ndarray]]:
        """Generate new command channels."""
        new_channels = []
        new_units = []
        new_data = []
        ...
        for chan_name, targets in cls.CMD_TARGETS.items():
            ...
            if chan_name not in channels:
                logger.warning(f"Source channel {chan_name} not found, skipping {cmd_name}")
                continue
    ...
    @classmethod
    def _discretize_channel(cls, values: np.ndarray, targets: List[float],
    ...
    def get_cmd_channel_info(cls, channel: str, unit_system: str) -> Optional[Dict]:
        ...
        base_channel = channel.replace('Cmd', '') if channel.startswith('Cmd') else channel
    ...
    @classmethod
    def validate_cmd_channels(cls, channels: List[str], data: np.ndarray) -> Dict[str, bool]:
        """
