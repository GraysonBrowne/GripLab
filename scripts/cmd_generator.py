        ...
        try:
            ...
            for chan, targets in cls.cmd_target.items():
                ...
                if unit_system not in targets:
                    raise ValueError(
                        f"Unsupported unit system {unit_system} for {chan}"
                    )
                ...
                if chan == "FZ":
                    values = low_pass_filter(
                        data_sae[:, col_idx], cutoff_hz=1, fs=100, order=2
                    )
                else:
                ...
                nearest = target_arr[
                    np.abs(values[:, None] - target_arr).argmin(axis=1)
                ]
            ...
            result = ConventionConverter.convert_channel_convention(
                channels,
                data_sae,
