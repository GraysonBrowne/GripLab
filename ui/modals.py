...
def create_settings_layout(settings_widgets, save_callback, dir_callback):
    ...
    return pn.Column(
        ...
        pn.Row(
            ...
            pn.Column(
                pn.widgets.StaticText(value="Demo Mode"),
                settings_widgets.demo_switch
            )
        ),
        pn.Row(
            settings_widgets.unit_select,
            settings_widgets.sign_select
        ),
        pn.Row(
            settings_widgets.data_dir_btn,
            settings_widgets.data_dir_input
        ),
        pn.Row(
            pn.layout.HSpacer(),
            settings_widgets.save_button
        ),
        width=800,
        margin=(0, 20)
    )
