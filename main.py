...
colorway = (
    px.colors.qualitative.__getattribute__(color)
    if hasattr(px.colors.qualitative, color)
    else px.colors.qualitative.G10
)
...
settings_button = pn.widgets.Button(name="Settings", button_type="primary", width=100)
# Header layout
...
model_table = pn.widgets.Tabulator(
...
plot_settings_button = pn.widgets.Button(
    name="⚙️", button_type="default", width=43, margin=(5, 0, 5, 15)
)
plot_data_button = pn.widgets.Button(
    name="Plot Data",
    button_type="primary",
    sizing_mode="stretch_width",
    margin=(5, 10, 5, 7),
)
plot_radio_group = pn.widgets.RadioBoxGroup(
...
x_select = pn.widgets.Select(name="X-Axis", options=[], sizing_mode="stretch_width")
y_select = pn.widgets.Select(name="Y-Axis", options=[], sizing_mode="stretch_width")
...
cmd_select_1 = pn.widgets.Select(
    name="Conditional Parsing", options=[], sizing_mode="stretch_width", min_width=80
)
cmd_select_2 = pn.widgets.Select(
    name=" ",
    options=[],
    sizing_mode="stretch_width",
    min_width=80,
...
cmd_select_3 = pn.widgets.Select(
    name=" ",
    options=[],
    sizing_mode="stretch_width",
    min_width=80,
...
cmd_select_4 = pn.widgets.Select(
    name=" ",
    options=[],
    sizing_mode="stretch_width",
    min_width=80,
...
cmd_multi_select_1 = pn.widgets.MultiSelect(
    options=[], size=8, sizing_mode="stretch_width", height=100, min_width=80
)
cmd_multi_select_2 = pn.widgets.MultiSelect(
    options=[], size=8, sizing_mode="stretch_width", height=100, min_width=80
)
cmd_multi_select_3 = pn.widgets.MultiSelect(
    options=[], size=8, sizing_mode="stretch_width", height=100, min_width=80
)
cmd_multi_select_4 = pn.widgets.MultiSelect(
    options=[], size=8, sizing_mode="stretch_width", height=100, min_width=80
)
downsample_slider = pn.widgets.IntSlider(
    name="Down Sample Rate",
    start=1,
    end=10,
    step=1,
    value=5,
...
node_count_text = pn.widgets.StaticText(
    name="Node Count",
    value="0",
    sizing_mode="stretch_width",
)
# data info widgets
data_select = pn.widgets.Select(
    name="Dataset",
    options=[],
    sizing_mode="stretch_width",
)
data_name_text_input = pn.widgets.TextInput(
    name="Name", sizing_mode="stretch_width", disabled=True
)
node_color_picker = pn.widgets.ColorPicker(name="Color", disabled=True)
tire_id_text_input = pn.widgets.TextInput(
    name="Tire ID", sizing_mode="stretch_width", disabled=True
)
rim_width_text_input = pn.widgets.IntInput(
    name="Rim Width [in]", width=100, disabled=True
)
notes_area_input = pn.widgets.TextAreaInput(
    name="Notes", sizing_mode="stretch_width", disabled=True
)
update_data_button = pn.widgets.Button(
    name="Update Dataset",
    button_type="primary",
    sizing_mode="stretch_width",
    disabled=True,
...
# Sidebar layout
plot_data_tab = pn.Column(
    pn.Row(plot_radio_group, plot_settings_button, plot_data_button),
    pn.Row(
        pn.GridBox(
            x_select,
            y_select,
...
data_info_tab = pn.Column(
    data_select,
    pn.Row(
        data_name_text_input,
        node_color_picker,
    ),
    pn.Row(
        tire_id_text_input,
        rim_width_text_input,
    ),
    notes_area_input,
    update_data_button,
    name="Data Info",
...
# Modals
# Settings widgets
default_theme_select = pn.widgets.Select(
    name="Default Theme",
...
colorway_select = pn.widgets.ColorMap(
    name="Color Sequence",
...
data_dir_button = pn.widgets.Button(
...
save_settings_button = pn.widgets.Button(
    name="Save Settings", button_type="primary", margin=(10, 15, 0, 15), width=200
)
# Remove confirmation widgets
confirm_remove_data = pn.pane.HTML(
    """""",
    styles={
        "font-size": "16px",
    },
...
cancel_remove_button = pn.widgets.Button(
    name="Cancel", button_type="default", margin=(10, 10, 0, 10), width=200
)
confirm_remove_button = pn.widgets.Button(
    name="Remove Dataset", button_type="primary", margin=(10, 10, 0, 10), width=200
)
# Plot settings widgets
title_text_input = pn.widgets.TextInput(
    name="Title", value="", placeholder="Tire ID", sizing_mode="stretch_width"
)
subtitle_text_input = pn.widgets.TextInput(
    name="Subtitle",
    value="",
    placeholder="SA:() | SR:() | IA:() | FZ:() | P:() | V:() | Rim Width:()",
    sizing_mode="stretch_width",
)
x_label_text_input = pn.widgets.TextInput(
    name="X-Axis Label",
    value="",
    placeholder="Channel [unit]",
    sizing_mode="stretch_width",
)
y_label_text_input = pn.widgets.TextInput(
    name="Y-Axis Label",
    value="",
    placeholder="Channel [unit]",
    sizing_mode="stretch_width",
)
z_label_text_input = pn.widgets.TextInput(
    name="Z-Axis Label",
    value="",
    placeholder="Channel [unit]",
    sizing_mode="stretch_width",
...
c_label_text_input = pn.widgets.TextInput(
    name="Colorbar Label",
    value="",
    placeholder="Channel [unit]",
    sizing_mode="stretch_width",
...
font_size_input = pn.widgets.IntSlider(
    name="Font Size", value=18, start=4, end=32, step=1, sizing_mode="stretch_width"
)
marker_size_input = pn.widgets.IntSlider(
    name="Marker Size", value=10, start=2, end=18, step=1, sizing_mode="stretch_width"
)
color_map_options = {
    "Jet": [
        "#010179",
...
settings_layout = pn.Column(
    ...
    pn.Row(
        default_theme_select,
        colorway_select,
        pn.Column(pn.widgets.StaticText(value="Demo Mode"), demo_switch),
    ),
...
plot_settings_layout = pn.Column(
    pn.pane.HTML(
        """<h1>Plot Settings</h1>""",
        styles={
            "height": "40px",
...
px.defaults.template = (
    "plotly_dark" if template.theme.name == "DarkTheme" else "plotly_white"
)
...
def update_colormap(event):
    selection = list(color_map_options.keys())[
        list(color_map_options.values()).index(event)
    ]
...
def update_demo_mode(event):
    ...
    if event:
        data_table.value = pd.DataFrame(
            {"Dataset": dm.list_demo_names(), "": [""] * len(dm.list_demo_names())}
        )
        data_select.options = [""] + dm.list_demo_names()
    else:
        data_table.value = pd.DataFrame(
            {"Dataset": dm.list_datasets(), "": [""] * len(dm.list_datasets())}
        )
...
def import_data(clicks):
    ...
    for file_path in files:
        ...
        colorway = px.colors.qualitative.__getattribute__(
            config["plotting"]["colorway"]
        )
...
def remove_data(clicks):
    ...
    color_select.options = channels
    logger.debug(
        f"Axis selectors updated: x:{x_select.options}, y:{y_select.options},"
        f" z:{z_select.options}, color:{color_select.options}"
    )
    # Update command channel options
    update_cmd_options(event=None)
...
def update_scatter_plot(clicks):
    if data_table.selection == []:
        ...
        return
    plotly_pane.object, node_count = PlottingUtils.plot_data(
        data_table,
        dm,
        x_select,
        y_select,
        ...
        for i, sel in enumerate(selectors):
            ...
            for name in names:
                ...
                dataset = UnitSystemConverter.convert_dataset(
                    dataset, to_system=unit_select.value
                )
                dataset = ConventionConverter.convert_dataset_convention(
                    dataset, target_convention=sign_select.value
                )
                data.extend(
                    dataset.data[:, dataset.channels.index(sel.value)]
                    if sel.value in dataset.channels
                    else []
                )
...
def update_data_info(clicks):
    ...
    ----------
    clicks : int
        Number of times the update button was clicked (unused, but required by callback
            signature).
    """
    ...
    try:
        ...
        if demo_switch.value:
            ...
            logger.info(
                "Updated dataset '%s' with new widget values.", dataset.demo_name
            )

            # Refresh the data table
            datasets = dm.list_demo_names()
            data_table.value = pd.DataFrame(
                {"Dataset": datasets, "": ["" for _ in datasets]}
            )
        else:
            # Update data manager
        ...
        logger.error(
            "Failed to update dataset '%s': %s", data_select.value, e, exc_info=True
        )
