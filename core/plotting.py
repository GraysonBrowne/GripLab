...
class PlotBuilder:
    ...
    @staticmethod
    def add_2d_color_trace(fig: go.Figure, data: PlotData, config: PlotConfig,
    ...
    @staticmethod
    def add_3d_color_trace(fig: go.Figure, data: PlotData, config: PlotConfig,
                          color_range: Tuple[float, float]) -> None:
...
class DataProcessor:
    ...
    @staticmethod
    def prepare_dataset(dataset: Any, config: PlotConfig,
                       cmd_filters: Optional[Dict[str, List]] = None) -> Any:
        """
        ...
        if cmd_filters:
            for channel, values in cmd_filters.items():
                if channel and values:
                    logger.debug(f"Filtering {dataset.name} on {channel} for values {values}")
                    dataset = DataProcessor._filter_by_channel(
...
class PlotMetadataBuilder:
    ...
    @staticmethod
    def build_title(datasets: List[Any], config: PlotConfig,
...
class PlottingUtils:
    ...
    def plot_data(cls, data_table, dm, x_select, y_select, z_select, color_select,
                 ...
                 cmd_multi_select_1, cmd_multi_select_2, cmd_multi_select_3, cmd_multi_select_4,
                 axis_visibility, title_text="", subtitle_text="",
                 x_label_text="", y_label_text="", z_label_text="", c_label_text="",
                 font_size=18, marker_size=10) -> Tuple[go.Figure, int]:
        """
        ...
        config = PlotConfig(
            ...
            color_channel=color_select.value if "Color" in plot_radio_group.value else None,
            title=title_text,
        ...
        if "Color" in config.plot_type.value:
            all_colors = [pd.c for pd in plot_data_list if pd.c is not None and len(pd.c) > 0]
            if all_colors:
                color_range = (
        ...
        # Build metadata
        config.title = PlotMetadataBuilder.build_title(datasets, config, axis_visibility)
        config.subtitle = PlotMetadataBuilder.build_subtitle(datasets, config)
        
        # Get units for labels
        if datasets:
            config.x_unit = datasets[0].units[datasets[0].channels.index(config.x_channel)]
            config.y_unit = datasets[0].units[datasets[0].channels.index(config.y_channel)]
            ...
            if config.z_channel:
                config.z_unit = datasets[0].units[datasets[0].channels.index(config.z_channel)]
                config.z_label = PlotMetadataBuilder.build_axis_label(
                    config.z_channel, config.z_unit, config.z_label
                )
            
            if config.color_channel:
                config.color_unit = datasets[0].units[datasets[0].channels.index(config.color_channel)]
    ...
    def _build_cmd_filters(selectors: List, multi_selectors: List) -> Dict[str, List]:
        ...
        for selector, multi in zip(selectors, multi_selectors):
            if selector.value and multi.value:
                ...
                selected_values = [
                    int(k) for k, v in multi.options.items()
                    if v in multi.value
                ]
