...
@dataclass
class Dataset:
    """Container for tire test dataset with metadata."""
...
class DataManager:
    ...
    def parse_dataset(self, dataset: Dataset, channel: str, 
                     condition: List[Any]) -> Optional[Dataset]:
...
class DataImporter:
    ...
    @staticmethod
    def import_file(filepath: Path, name: str, node_color: str, 
                   demo_name: str) -> Optional[Dataset]:
    ...
    @staticmethod
    def import_mat(filepath: Path, name: str, node_color: str, 
                  demo_name: str) -> Optional[Dataset]:
        ...
        try:
            ...
            channels = np.concatenate(
                file_data['channel'][0][0][0][0]
            ).ravel().tolist()
            units = np.concatenate(
                file_data['channel'][0][0][1][0]
            ).ravel().tolist()
    ...
    @staticmethod
    def import_dat(filepath: Path, name: str, node_color: str,
                  demo_name: str) -> Optional[Dataset]:
        ...
        try:
            ...
            # Extract metadata
            metadata = DataImporter._extract_dat_metadata(
                header_lines[0], name, units
            )
    ...
    def _extract_mat_metadata(file_data: Dict, name: str) -> Dict[str, Any]:
        ...
        if 'units' in file_data:
            ...
            if 'channel' in file_data:
                ...
                metadata['unit_system'] = (
                    'USCS' if 'lb' in str(units) else 'Metric'
                )
            logger.warning(
        ...
        if 'sign' in file_data:
            ...
            logger.warning(
                f"Sign convention not specified in {name}, "
                f"defaulting to SAE"
            )
    ...
    @staticmethod
    def _extract_dat_metadata(header_line: str, name: str, units: List[str]) -> Dict[str, Any]:
...
def import_mat(filepath, file_name, node_color, demo_name):
    ...
    return DataImporter.import_mat(
        Path(filepath), file_name, node_color, demo_name
    )


def import_dat(filepath, file_name, node_color, demo_name):
    """Legacy function for importing DAT files."""
    return DataImporter.import_dat(
        Path(filepath), file_name, node_color, demo_name
    )
